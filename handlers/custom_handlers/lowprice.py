...
def bot_start(message: Message) -> None:
    """
    Обработчик комманды бота "/lowprice"
    ...
    """
    logger.info(f"User_id: {message.from_user.id} use /lowprice command")
    bot.set_state(message.from_user.id, RequestInfoState.city, message.chat.id)
    bot.send_message(message.from_user.id, f"Ищем самые дешевые отели. Введите город:")
    with bot.retrieve_data(message.from_user.id, message.chat.id) as request:
        request['command'] = 'lowprice'
