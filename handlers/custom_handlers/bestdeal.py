...
def bot_start(message: Message):
    """
    Обработчик комманды бота "/bestdeal"
    ...
    """
    logger.info(f"User_id: {message.from_user.id} use /bestdeal command")
    bot.set_state(message.from_user.id, RequestInfoState.city, message.chat.id)
    bot.send_message(message.from_user.id, f"Ищем подходящие отели. Введите город:")
    with bot.retrieve_data(message.from_user.id, message.chat.id) as request:
        request['command'] = 'bestdeal'
