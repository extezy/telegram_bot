...
def process_callback(query):
    ...
    # Обработчик события подтверждения данных
    if query.data == "request_approve":
        with bot.retrieve_data(query.from_user.id, query.message.chat.id) as request:
    ...
    # Обработчик событий выбора даты заезда / отъезда
    if query.data.startswith('Дата'):
        # День: 'Дата:DAY:2022:7:13'
        # Месяц: 'Дата:MONTHS:2022:7:!'
        # День недели или пустая: 'Дата:IGNORE:2022:7:!'
        # Отмена: 'Дата:CANCEL:2022:7:!'
        # 'Дата:NEXT-MONTH:2022:7:!'
        # 'Дата:PREVIOUS-MONTH:2022:7:!'
